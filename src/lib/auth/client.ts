...
export async function signIn(
  ...
  // Deployed site: native Google/X. Preview iframe still uses the Grok broker.
  const idp =
    GROK_PROVIDERS.find((p) => p.providerId === providerId)?.idp ?? providerId;
  const socialProvider = idp === "twitter" ? "twitter" : idp;
  const { data, error } = await authClient.signIn.social({
    provider: socialProvider as "google" | "twitter",
    callbackURL,
    errorCallbackURL,
  });
